 ...
 */
@CompileStatic
public class SQLDeleteTestSuite extends TestSuite {

    public SQLDeleteTestSuite(Context context) {
        super(context)
    }
    ...
    public void runTests(Closure onTestFinished) {
        ...
        NUMBER_OF_TESTS.times {
            ...
            Cursor users = dataSource.findAll()
            if (users.getCount() == 0) {
                dataSource.createUserBulk(OPERATIONS_PER_TEST)
                users = dataSource.findAll()
            }

            long startTime = System.currentTimeMillis()
            dataSource.deleteUsers(users)
            dataSource.close()
